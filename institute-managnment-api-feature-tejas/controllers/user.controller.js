...
// ✅ Define your secret key (fallback if env not loaded)
...
// Email sender: works in TEST and PROD
const sendMail = async (to, subject, message) => {
  if (process.env.NODE_ENV === "production") {
    ...
    // TEST: just log email instead of sending
...
export const login = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: user.user_id, email: user.email }, JWT_KEY, { expiresIn: "24h" });
...
export const getAllUsers = async (req, res) => {
  try {
    ...
    if (req.query.customerId) {
      whereClause.customer_id = req.query.customerId;
    }

    const users = await User.findAll({
      attributes: ['user_id', 'full_name', 'username', 'email', 'role_id', 'customer_id'],
      where: whereClause,
      include: [
        {
          model: Role,
          as: 'Role', // <-- Use the alias defined in your association
          attributes: ['role_id', 'role_name', 'role', 'permission', 'is_active', 'is_deleted', 'createdAt', 'updatedAt']
        },
        {
          model: Customer,
          as: 'Customer', // <-- Use the alias defined in your association
          attributes: ['customer_id', 'companyName', 'companyEmail', 'phone', 'CompanyAddress', 'city', 'state', 'is_active', 'is_deleted', 'created_at', 'updated_at']
        }
      ],
       order: [['createdAt', 'DESC']] // ✅ Fetch recent users first
    });

    // Log the response for debugging
    console.log("Fetched users:", users.map(u => u.dataValues));

    return response.successResponse(
      res,
      200,
      users.map(u => u.dataValues),
...
export const getUserById = async (req, res) => {
  try {
    const { id } = req.params;
    const user = await User.findByPk(id, { attributes: ['user_id', 'username', 'email', 'role_id','customer_id'] });
...
//     console.log("response sent to:", response.data);
//     return response.successResponse(res, 200, {}, HTTP_MESSAGES.EN.DATA_ADDED_SUCCESS);
//   } catch (error) {
//     console.error("Add user error:", error);
//     return response.errorMessageResponse(res, 500, {}, HTTP_MESSAGES.EN.SERVER_ERROR);
...
export const updateUser = async (req, res) => {
  try {
    ...
    if (customer_id) user.customer_id = customer_id;
    ...
    return response.successResponse(
      res,
      200,
      { user }, // optionally return updated user
...
export const deleteUser = async (req, res) => {
  try {
    ...
    if (!user) {
      return response.errorMessageResponse(res, 404, {}, "User not found");
    }
